...
class StockDataLoader:
    def __init__(
        ...
        train_size=0.8,train_test_method='random',seed=0,
        scaling=True,target_logscale=False,lag_length=3,
    ):
        assert train_test_method in ['timeseries','random'], \
            "train_test_method must be in ['timeseries','random']"
        ...
        self.train_data, self.test_data = self._train_test_split(
            self.data,train_size=train_size,train_test_method=train_test_method,seed=seed)
        
        # (4) scaling
        if scaling:
            self.train_data, self.test_data = self._scaling(
                self.train_data,
                self.test_data,
                feature_range=(0,1),
    ...
    def _train_test_split(self,data,train_test_method,train_size=0.8,seed=0):
        df = data.copy()
        
        if train_test_method=='timeseries':
            train_size = int(len(df) * train_size)
            test_size  = len(df) - train_size
            train_data, test_data = df[:train_size], df[train_size:]
        elif train_test_method=='random':
            train_data, test_data = train_test_split(data,train_size=train_size,random_state=seed)
        else:
            raise ValueError("train_test_method must be in ['timeseries','random']")
        
        return train_data, test_data

    def _scaling(self,train_data,test_data,feature_range=(0,1),ignore_features=[]):
